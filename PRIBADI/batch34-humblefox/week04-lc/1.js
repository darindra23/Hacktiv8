...
PSEUDOCODE:
-----------
STORE DISPLAY with 0 Value
DO LOOP WITH
STORE i With 0 Value
IF i mod 2 not equal to 0
DO Calculate StringNumber[i] * 2
ADD Display with calculation value
ELSE IF i mod 2 equal to 0
ADD display with StringNumber[i]
RETURN Display
================================================================
*/
...
function sumTheNumber (stringNumber) {
  ...
  for(i = 0; i < stringNumber.length; i++){
    if(i % 2 !== 0){
      var kali = Number(stringNumber[i]) * 2
      display += kali
    } else {
      display += Number(stringNumber[i])
    }
  }
  return display
}
